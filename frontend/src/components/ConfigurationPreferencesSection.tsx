...
interface ConfigurationPreferencesSectionProps {
  onStatsChange: (stats: OverviewStatsResponse | null) => void;
}

type StudyLanguageMessageKey =
...
function getStudyLanguageMessageKey(code: StudyLanguageCode): StudyLanguageMessageKey {
  return STUDY_LANGUAGE_MESSAGE_KEY_BY_CODE[code] as StudyLanguageMessageKey;
}

export default function ConfigurationPreferencesSection({
  onStatsChange,
  ...
  }, [onStatsChange, sourceLanguage, targetLanguage]);
      ...
      <div className="settings-grid">
        ...
        <div className="settings-field">
          ...
          <div className="settings-choice-group" role="radiogroup" aria-label={t("config.targetPromptMode")}>
            <button type="button" className={`settings-choice-button ${targetPromptMode === "text" ? "settings-choice-button-selected" : ""}`} onClick={() => setTargetPromptMode("text")} role="radio" aria-checked={targetPromptMode === "text"}>{t("config.targetPromptModeText")}</button>
        ...
        <div className="settings-field">
          ...
          <div className="settings-choice-group" role="radiogroup" aria-label={t("config.mobileActionLabels")}>
            <button type="button" className={`settings-choice-button ${showMobileActionLabels ? "settings-choice-button-selected" : ""}`} onClick={() => setShowMobileActionLabels(true)} role="radio" aria-checked={showMobileActionLabels}>{t("config.mobileActionLabelsOn")}</button>
        ...
        <div className="settings-field">
              ...
              {browserVoiceOptions.map((voice) => {
                ...
                return (
                  <div key={voice.voiceURI} className={`browser-voice-option ${selected ? "browser-voice-option-selected" : ""}`}>
                    <button type="button" className={`browser-voice-select-button ${selected ? "browser-voice-select-button-selected" : ""}`} onClick={() => setPreferredBrowserVoiceURI(targetLanguage, voice.voiceURI)} role="radio" aria-checked={selected}>
                      <span className="browser-voice-option-name">{voice.name}</span>
                      <span className="browser-voice-option-meta">{voice.lang}</span>
                    </button>
                    <button type="button" className="secondary-button browser-voice-preview-button" onClick={() => previewBrowserVoice(voice)} disabled={previewingVoiceURI === voice.voiceURI}>
          ...
          <span className="hint">{t("config.browserVoiceHint", { language: t(getStudyLanguageMessageKey(targetLanguage)) })}</span>
        </div>
      </div>
      <div className="actions">
        <button type="button" className="secondary-button" onClick={resetDefaults}>{t("config.resetDefaults")}</button>
      </div>
    </section>
  );
}
