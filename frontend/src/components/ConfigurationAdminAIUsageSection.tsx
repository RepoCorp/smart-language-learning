...
export default function ConfigurationAdminAIUsageSection({
  ...
  const updateQuotaInput = (
    userId: number,
    field: "weekly_generation_credits" | "weekly_elevenlabs_characters" | "weekly_elevenlabs_music_seconds" | "weekly_realtime_minutes",
    value: string,
  ): void => {
  ...
  return (
    <section className="card settings-card">
      ...
      <p className="settings-subtitle">
        Default weekly budget: {defaults.weekly_generation_credits} OpenAI generation credits, {defaults.weekly_elevenlabs_characters} ElevenLabs characters, {defaults.weekly_elevenlabs_music_seconds} Eleven Music seconds, and {defaults.weekly_realtime_minutes} live minutes.
        ...
        {users.map((user) => (
          <div className="ai-usage-row" key={user.id}>
            <div>
              ...
              <p className="hint">This week: {user.week_generation_credits} OpenAI generation credits, {user.week_elevenlabs_characters} ElevenLabs characters, {user.week_elevenlabs_music_seconds} Eleven Music seconds, {user.week_realtime_minutes} live minutes</p>
            </div>
            ...
            <label>
              Eleven Music seconds
              <input
                type="number"
                min="0"
                value={quotaInputValues[`${user.id}:weekly_elevenlabs_music_seconds`] ?? String(user.weekly_elevenlabs_music_seconds)}
                onChange={(event) => updateQuotaInput(user.id, "weekly_elevenlabs_music_seconds", event.target.value)}
                onBlur={() => restoreDefaultQuotaInput(user.id, "weekly_elevenlabs_music_seconds")}
              />
            </label>
            <label>
              Live minutes
