...
@pytest.mark.django_db
def test_partial_phrase_target_to_source_session_keeps_its_own_prompt_audio():
    now = timezone.now()
    phrase = Item.objects.create(
        item_type=Item.ItemType.PHRASE,
        spanish_text="entiendo",
        german_text="verstehe",
        source_language="spanish",
        target_language="german",
        audio_url="http://localhost:8000/media/audio/openai-partial-phrase.mp3",
        last_reviewed_at_de_to_es=now,
        due_at_de_to_es=now,
    )
    dialog = SavedDialog.objects.create(
    ...
    entry = response.json()["items"][0]
    payload_item = load_session_item_payload(client, entry)
    assert payload_item["prompt_audio_url"] == "http://localhost:8000/media/audio/openai-partial-phrase.mp3"
